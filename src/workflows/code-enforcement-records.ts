...
export const codeEnforcementRecordsWorkflow: RecordsWorkflow = createRecordsWorkflow({
  ...
  responseAnalysis:{findingTypes:ANALYSIS_FINDING_TYPES, async analyze(input:unknown){
    ...
    const ai=await runCodeEnforcementStrategy({requestedItems:source.requestedItems??[],productionSummary:{recordsReviewed:deterministic.recordsReviewed,findings:deterministic.findings,coveredCategoryIds:deterministic.coveredCategoryIds,missingCategoryIds:deterministic.missingCategoryIds},records:records.map(record=>({id:record.id,filename:record.filename,category:record.category,text:record.text??''}))})
    return {...deterministic,aiStrategy:ai.value,aiProvenance:{providers:ai.providers,agreement:ai.confidence,disagreements:ai.disagreements,warnings:ai.warnings}}
  }},
})
