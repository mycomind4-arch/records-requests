...
function normalizeRequestedCategories(items: readonly {category:string;description:string}[]): RequestedCategory[] { return items.map(item=>({id:item.category,label:item.category,keywords:item.description.split(/\W+/).filter(Boolean).slice(0,12)})) }
