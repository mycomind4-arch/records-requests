...
function buildCategories(input: Record<string, unknown>): string[] {
  ...
  const selected = requested.filter((entry): entry is string => typeof entry === 'string' && known.has(entry as typeof CODE_ENFORCEMENT_RECORD_CATEGORIES[number]))
...
function descriptionForCategory(category: string, input: Record<string, unknown>): string {
  ...
  const subjectText = subject ? ` The matter concerns: ${subject}.` : ''
...
function validateCodeEnforcement(request: ValidatedRequest): readonly { field: string; message: string }[] {
  ...
  const hasProperty = scope.some((item) => item.description.toLowerCase().includes('property address'))
  const hasCase = scope.some((item) => item.description.toLowerCase().includes('case number'))
  const hasSubject = scope.some((item) => item.description.toLowerCase().includes('matter concerns'))

  if (!hasProperty && !hasCase) {
    issues.push({ field: 'identifiers', message: 'Provide a property address or case number so the agency can identify the enforcement matter.' })
  }
  if (!hasSubject) {
  ...
  if (!scope.some((item) => item.category === 'case-file')) {
    issues.push({ field: 'categories', message: 'The flagship workflow should include the case-file category unless the user deliberately narrows the request.' })
  }
  return issues
}
...
export function buildCodeEnforcementRequest(input: Record<string, unknown>) {
  ...
  return {
    ...
    purpose: value(input, 'purpose') ?? 'Research and document the code-enforcement history and agency records for the identified matter.',
...
export const codeEnforcementRecordsWorkflow: RecordsWorkflow = createRecordsWorkflow({
  ...
  description: 'Build a targeted records request for code violations, complaints, inspections, notices, photographs, correspondence, enforcement actions, and property-linked case records.',
  searchIntent: 'code enforcement records request',
  seo: {
    title: 'Code Enforcement Records Request',
    description: 'Build a targeted code enforcement records request for a property, parcel, violation, complaint, or enforcement case.',
  ...
  request: {
    categories: CODE_ENFORCEMENT_RECORD_CATEGORIES,
    build: buildCodeEnforcementRequest,
  },
  ...
  responseAnalysis: {
    findingTypes: [
      ...
      'DUPLICATE_RECORD',
      'MISSING_ATTACHMENT',
      'INCONSISTENT_CASE_IDENTIFIER',
      'INCONSISTENT_PROPERTY_IDENTIFIER',
