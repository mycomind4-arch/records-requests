...
function matches(r:PropertyPermitProductionRecord,c:PropertyPermitRequestedCategory){const h=content(r).toLowerCase(); return c.keywords.some(k=>h.includes(k.toLowerCase()))}
export function analyzePropertyPermitProduction(requested:readonly PropertyPermitRequestedCategory[],records:readonly PropertyPermitProductionRecord[]):PropertyPermitProductionAnalysis{
 ...
 for(const r of records){const t=content(r); if(r.sha256){const prior=hashes.get(r.sha256); if(prior) findings.push({id:`duplicate-${r.id}`,type:'DUPLICATE_RECORD',severity:'info',description:`Record appears to duplicate produced record ${prior} by SHA-256.`,recordIds:[prior,r.id]}); else hashes.set(r.sha256,r.id)}
  ...
  if(REFERENCE.test(t) && !/\.(pdf|docx?|xlsx?|csv|jpg|jpeg|png|mp4|mov)$/i.test(r.filename)) findings.push({id:`reference-${r.id}`,type:'REFERENCED_RECORD_NOT_PRODUCED',severity:'warning',description:`Record ${r.filename} appears to reference another property/permit record; confirm whether that referenced material was separately produced.`,recordIds:[r.id]})
 }
