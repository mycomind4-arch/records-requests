...
const provider = (id: string, value: unknown, confidence = 0.9): LlmProvider => ({
  id,
  complete: async () => ({ provider: id, model: `${id}-model`, value, confidence, warnings: [] }),
})
...
describe('multi-LLM orchestrator', () => {
  ...
  it('surfaces disagreement instead of pretending certainty', async () => {
    const result = await runMultiLlm(
      ...
      'classification',
      {},
      { minimumProviders: 2, agreementThreshold: 0.8, maxProviders: 3 },
    )
