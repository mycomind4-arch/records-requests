...
export default async function WorkflowPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const builderPath = slug === 'code-enforcement-records' ? `/workflows/${slug}/build` : slug === 'police-records' ? `/workflows/${slug}/builder` : '/dashboard'
  ...
  return <main className="landing workflowPage">
    ...
    <section className="section workflowSection"><div><div className="eyebrow">SEARCH-READY SCOPE</div><h2>Build the request around the records job, not a vague request for “everything.”</h2><p className="workflowCopy">A strong records request identifies the agency, record type, time period, and identifiers or custodians that make the records searchable. This workflow turns those inputs into a structured request.</p></div><div className="workflowList">{workflow.bestFor.map((item,index)=><div className="workflowItem" key={item}><span>{String(index+1).padStart(2,'0')}</span><strong>{item}</strong></div>)}</div></section>
    {isPolice && <section className="section workflowSection"><div><div className="eyebrow">POLICE RECORD TYPES</div><h2>Ask for the records system by system.</h2><p className="workflowCopy">Depending on the agency and applicable law, a targeted request can cover incident reports, CAD or dispatch records, body-camera and dash-camera recordings, 911 calls, photographs, witness statements, supplements, and case correspondence.</p></div><div className="workflowList">{['Incident and offense reports','CAD / dispatch records','Body- and dash-camera media','911 calls and communications','Photographs and scene media','Witness, victim and supplemental records'].map((item,index)=><div className="workflowItem" key={item}><span>{String(index+1).padStart(2,'0')}</span><strong>{item}</strong></div>)}</div></section>}
    {isCode && <><section className="section workflowSection"><div><div className="eyebrow">CODE ENFORCEMENT RECORD TYPES</div><h2>Target the records that explain what happened at the property.</h2><p className="workflowCopy">Depending on the agency and applicable law, a focused request can identify complaints, inspections, photographs or video, notices of violation, correction notices, citations, administrative orders, abatement records, permits, correspondence, and other identifiable enforcement records.</p></div><div className="workflowList">{['Case file and routing history','Complaints and service requests','Inspections and inspection reports','Photographs and video','Notices, citations and orders','Correspondence and enforcement communications','Enforcement actions','Abatement and compliance','Permits and related records'].map((item,index)=><div className="workflowItem" key={item}><span>{String(index+1).padStart(2,'0')}</span><strong>{item}</strong></div>)}</div></section><section className="section workflowSection"><div><div className="eyebrow">PROPERTY IDENTIFIERS</div><h2>Address, parcel, case number, and dates matter.</h2><p className="workflowCopy">Code-enforcement records are often organized around property and case identifiers. Include whatever is known rather than guessing at an unknown case number.</p></div><div className="workflowList">{['Property address','Parcel / APN','Case / violation number','Related person or entity','Department / custodian','Date range'].map((item,index)=><div className="workflowItem" key={item}><span>{String(index+1).padStart(2,'0')}</span><strong>{item}</strong></div>)}</div></section></>}
