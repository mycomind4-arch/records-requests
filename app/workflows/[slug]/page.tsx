...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) { const { slug }=await params; const workflow=workflows.find(item=>item.slug===slug); if(!workflow)return {}; return { title:`${workflow.title} | Records Requests`,description:workflow.description,alternates:{canonical:`/workflows/${workflow.slug}`},openGraph:{title:workflow.title,description:workflow.description} } }

export default async function WorkflowPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug }=await params; const workflow=workflows.find(item=>item.slug===slug); if(!workflow)notFound(); const builderPath=slug==='code-enforcement-records'?'/workflows/code-enforcement-records/builder':'/dashboard'
  return <main className="landing workflowPage">
    ...
    <section className="workflowHero"><div className="eyebrow">{workflow.category} · Search intent: {workflow.intent}</div><h1>{workflow.title}</h1><p className="lede">{workflow.description}</p><div className="cta"><Link className="primary" href={builderPath}>{workflow.cta} →</Link><Link className="secondary" href="/">Browse every records workflow</Link></div></section>
    ...
    <section className="final"><div className="eyebrow">READY TO BUILD IT</div><h2>Turn this search into an actual request.</h2><p>Start with the objective in plain English. The workspace handles the structure.</p><Link className="primary" href={builderPath}>Open the request builder →</Link></section>
  </main>
}
