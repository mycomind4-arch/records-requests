...
export default async function WorkflowPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const builderSlugs = new Set(['code-enforcement-records', 'police-records', 'property-records', 'permit-inspection-records', 'government-communications-records', 'court-records', 'foia-request'])
  const builderPath = builderSlugs.has(slug) ? `/workflows/${slug}/builder` : '/dashboard'
  ...
  const jsonLd = {
    ...
    '@graph': [
      { '@type': 'WebSite', '@id': 'https://records-requests.example/#website', name: 'Records Requests', url: '/' },
      { '@type': 'WebPage', '@id': `/workflows/${workflow.slug}#webpage`, name: workflow.seo?.title ?? workflow.title, description: workflow.seo?.description ?? workflow.description, url: `/workflows/${workflow.slug}`, isPartOf: { '@id': 'https://records-requests.example/#website' }, about: { '@type': 'Thing', name: workflow.intent } },
  ...
  return <main className="landing workflowPage">
    ...
    <section className="workflowHero">
      <div className="eyebrow">{workflow.category} · {workflow.intent}</div>
      <h1>{workflow.title}</h1>
      <p className="lede">{workflow.description}</p>
      <div className="cta"><Link className="primary" href={builderPath}>{workflow.cta} →</Link><Link className="secondary" href="/">Browse every records workflow</Link></div>
      <div className="trust">Specific scope · Search-ready identifiers · Evidence-first tracking · Production audit ready</div>
    </section>

    <section className="section workflowSection">
      <div><div className="eyebrow">SEARCH-READY SCOPE</div><h2>Build the request around the records job, not a vague request for “everything.”</h2><p className="workflowCopy">A strong records request identifies the agency, the record type, the relevant time period, and the identifiers or custodians that make the records searchable. Official agencies routinely ask for details such as addresses, parcel or case numbers, dates, permit numbers, custodians, and a clear description of the records sought. The workflow turns those practical search inputs into a structured request instead of leaving them as scattered notes.</p></div>
      <div className="workflowList">{workflow.bestFor.map((item,index)=><div className="workflowItem" key={item}><span>{String(index+1).padStart(2,'0')}</span><strong>{item}</strong></div>)}</div>
    </section>

    {isPolice && <>
      <section className="section workflowSection"><div><div className="eyebrow">POLICE RECORD TYPES</div><h2>Ask for the records system by system.</h2><p className="workflowCopy">Depending on the agency and applicable law, a targeted request can cover incident reports, arrest and booking records, CAD or dispatch records, body-camera and dash-camera recordings, 911 calls, photographs, witness statements, supplements, and case correspondence.</p></div><div className="workflowList">{['Incident and offense reports','CAD / dispatch records','Body- and dash-camera media','911 calls and communications','Photographs and scene media','Witness, victim and supplemental records'].map((item,index)=><div className="workflowItem" key={item}><span>{String(index+1).padStart(2,'0')}</span><strong>{item}</strong></div>)}</div></section>
    ...
    {isCode && <>
      <section className="section workflowSection"><div><div className="eyebrow">CODE ENFORCEMENT RECORD TYPES</div><h2>Target the records that explain what happened at the property.</h2><p className="workflowCopy">Depending on the agency and applicable law, a focused request can identify complaints, inspections, photographs or video, notices of violation, correction notices, citations, administrative orders, abatement records, permits, correspondence, and other identifiable enforcement records.</p></div><div className="workflowList">{['Complaints and service requests','Inspections and inspection reports','Photographs and video','Notices, citations and orders','Permits and correction records','Correspondence and enforcement communications'].map((item,index)=><div className="workflowItem" key={item}><span>{String(index+1).padStart(2,'0')}</span><strong>{item}</strong></div>)}</div></section>
      <section className="section workflowSection"><div><div className="eyebrow">PROPERTY IDENTIFIERS</div><h2>Address, parcel, case number, and dates matter.</h2><p className="workflowCopy">Code-enforcement records are often organized around property and case identifiers. Include whatever is known rather than guessing at an unknown case number.</p></div><div className="workflowList">{['Property address','Parcel / APN','Case / violation number','Date range'].map((item,index)=><div className="workflowItem" key={item}><span>{String(index+1).padStart(2,'0')}</span><strong>{item}</strong></div>)}</div></section>
    </>}

    {isPermit && <section className="section workflowSection"><div><div className="eyebrow">PROPERTY & PERMIT SEARCH</div><h2>Address, APN, permit number, project and dates create a better permit search.</h2><p className="workflowCopy">Permit research commonly uses the site address, parcel/APN, permit number, permit type, project description and date range. Historical records may live in different systems or archives, so the request should state the range and record types precisely rather than assuming one database contains everything.</p></div><div className="workflowList">{['Site address','Parcel / APN','Permit or IVR number','Permit type','Project / applicant','Date range'].map((item,index)=><div className="workflowItem" key={item}><span>{String(index+1).padStart(2,'0')}</span><strong>{item}</strong></div>)}</div></section>}

    {isCommunications && <section className="section workflowSection"><div><div className="eyebrow">COMMUNICATION SEARCH</div><h2>Custodian + date range + distinctive terms beats “all emails.”</h2><p className="workflowCopy">For communications, the highest-value search inputs are usually the people or offices involved, the date range, the subject or project, distinctive names or phrases, known outside parties, and the systems that may contain responsive messages. Attachments and related records should be identified explicitly so they can be checked separately during production review.</p></div><div className="workflowList">{['Likely custodians','Date range','Project / subject','Distinctive keywords','Outside parties','Known systems'].map((item,index)=><div className="workflowItem" key={item}><span>{String(index+1).padStart(2,'0')}</span><strong>{item}</strong></div>)}</div></section>}

    <section className="section workflowSection"><div><div className="eyebrow">WHAT YOU ACTUALLY DO</div><h2>Turn the search into a request that an agency can act on.</h2><p className="workflowCopy">Enter what you know, identify the records, review the generated scope, and preserve the final request. The goal is not legal theater. It is a clear, searchable description of the records you want and the information that helps the custodian find them.</p></div><div className="workflowList"><div className="workflowItem"><span>01</span><strong>Describe the matter</strong></div><div className="workflowItem"><span>02</span><strong>Select records and scope</strong></div><div className="workflowItem"><span>03</span><strong>Review and validate</strong></div><div className="workflowItem"><span>04</span><strong>Approve and mail</strong></div></div></section>
