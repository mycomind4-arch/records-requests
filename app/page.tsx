...
export default function Home() {
  return <main className="landing directoryPage">
    ...
    <section className="directoryHero">
      ...
      <p className="lede">Start with the objective in plain English. Find the closest records job, build a focused request, then track the response and what was actually produced.</p>
    ...
    <section id="workflows" className="section directorySection"><div className="eyebrow">WORKFLOW DIRECTORY</div><h2>Search by the records job, not a giant list of pages.</h2><p className="directoryIntro">Each workflow is organized around a distinct record type and search intent. The directory can grow to hundreds of workflows without becoming a wall of cards.</p><WorkflowDirectory workflows={workflows.map(({ slug, title, description, category }) => ({ slug, title, description, category }))} groups={groups} /></section>
    ...
    <section className="final"><div className="eyebrow">START ANYWHERE</div><h2>You do not need to know the exact workflow name.</h2><p>Describe what you are trying to find. The directory helps narrow it down, and the workspace handles the structure.</p><Link className="primary" href="/dashboard">Open the Records workspace →</Link></section>
